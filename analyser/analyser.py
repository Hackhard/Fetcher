...
class Analyser:
    ...
    def remove_GDPR_popups_tor(self):
        clas = ""
        ...
        self.tor_driver.save_screenshot(self.abs_path+"/GDPRt.png")
        if(len(clas)>0):
            print("Removing GDPR: ",clas)
            ele = self.tor_driver.find_element_by_xpath('//*[text()="'+clas+'"]').click()
            

        self.tor_driver.refresh()
        self.tor_driver.save_screenshot(self.abs_path+"/GDPR_t.png")
        self.tor_page_source_sel_after_gdpr_removal = self.tor_driver.page_source
        # self.tor_driver.quit()


    def remove_GDPR_popups_non_tor(self):
        clas = ""
        # self.soup_t = BeautifulSoup(self.non_tor_page_source_sel, 'html.parser')  
        for tag in self.soup_n.find_all(True):
            sear = tag.get_text()
            if sear in self.gdpr_word_list:
                clas = tag.text

        self.non_tor_driver.save_screenshot(self.abs_path+"/GDPRn.png")
        if(len(clas)>0):
            print("Removing GDPR: ",clas)
            ele = self.non_tor_driver.find_element_by_xpath('//*[text()="'+clas+'"]').click()
            # This is done if a different website is redirected in place of the same (Youtube) would be an example
            time.sleep(25)
            
        self.non_tor_driver.refresh()
        self.non_tor_driver.save_screenshot(self.abs_path+"/GDPR_n.png")
        self.non_tor_page_source_sel_after_gdpr_removal = self.non_tor_driver.page_source
    ...
    def setup_tor(self):
        ...
        try:
            ...
            self.tor_driver = webdriver.Firefox(firefox_profile=profile,
            ...
            try:
                ...
                # print(dic_har)js = json.dumps({"log": har_dict})
                print("Tor Dumps")
                dic = json.loads(js)
                ...
                for i in range(len(dic['log']['entries'])):
                    ...
                    # print(dic['log']['entries'][i]['request']['url'],"  ",dic['log']['entries'][i]['response']['status'])
                print(term.format("1st Har stats (TOR)",term.Color.YELLOW))   
                print(dic['log']['entries'][0]['request']['url'],"  ",dic['log']['entries'][0]['response']['status'])
                f.write(str(dic))
                # dic_har = driver.execute_async_script("HAR;")
                # print(dic_har)
                f.close()
    ...
    def setup_non_tor(self):
        ...
        self.remove_GDPR_popups_non_tor()
        try:
            ...
            f = open(self.abs_path+"/Non_tor_exportHTTP.status", "w")
            for i in range(len(dic['log']['entries'])):
    ...
    def status_checker(self):
        ...
        status_url = next(iter(self.non_tor_HAR))
        sc = self.non_tor_HAR[str(status_url)]
        if self.non_tor_status_code > 399:
            ...
            if sc == self.non_tor_status_code or (int(sc) > 399 and self.non_tor_status_code > 399):
                ...
                print("It hasn't been thought of for Non-Tor")
                self.non_tor_status_code = sc
            else:
                self.non_tor_status_code = sc

        # Check if request library on tor fails or not:
        status_url_t = next(iter(self.tor_HAR))
        sc_t = self.tor_HAR[str(status_url_t)]
        ...
        if self.non_tor_status_code != self.tor_status_code:
            ...
            if(dom_score != 0):
                ...
                self.soup_t = BeautifulSoup(self.tor_page_source_sel_after_gdpr_removal,"html.parser")
                self.soup_n = BeautifulSoup(self.non_tor_page_source_sel_after_gdpr_removal, "html.parser")
                ...
                print("Nodes by tor and non-tor: (AFTER)")
                ...
                print("New Dom Score: ", dom_score)
                soup_t = str(self.soup_t).lower()
                soup_n = str(self.soup_n).lower()
