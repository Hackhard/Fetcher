...
def check(soup_t, soup_n ,count_t, count_n):
    ...
    if(dom_score > 0):
        if( dom_score > 150):   
            ...
            if "error" in soup_t and "error" not in soup_n or "forbidden" in soup_t and "forbidden" not in soup_n or "tor" in soup_t and "tor" not in soup_n or "denied" in soup_t and "denied" not in soup_n:
                print("Tor blocked : checklist")
            elif "captcha" in soup_t and "captcha" not in soup_n:
        ...
        if (dom_score < 0):
            print("Tor works Better in this case!! :P or website has cookie-popups ")
        else: 
...
def work(url):
    ...
    # BS4 prettify
    soup_t = BeautifulSoup(all_text, 'html.parser')
    ...
    dom_nodes = path+"/tor_["+url.replace("/", "")+"]_DOM_nodes.txt"
    ...
    # print("Number of Tor Nodes : ", count)
    f.close()
    ...
    # BS4 prettify
    soup_n = BeautifulSoup(all_text, 'html.parser')
    ...
    dom_nodes_non = path+"/non-tor_["+url.replace("/", "")+"]_DOM_nodes.txt"

    f = open(dom_nodes_non, "w")
    f.write("All Nodes (Non-Tor): \n")
    count_n = 0
    for tag in soup_n.find_all(True):
        count_n += 1
        # print(i, " -- ", tag.name)  # Print the names of DOM nodes
        f.write("{}--->{}\n".format(count_n, tag.name))

    f.write("\nTotal Number of Non-Tor Nodes: {}\n".format(count_n))
    f.close()
    ...
    if tbb_map[key_t] == nbb_map[key_nt]:
        ...
        if(int(tbb_map[key_t])>399):
            ...
            # else:
            #     if (dom_score < 0):
            #         print("Tor works Better in this case!! :) ")
            #     else: 
            #         print("Same Resemblance :D")
    print()
    print("----------")
